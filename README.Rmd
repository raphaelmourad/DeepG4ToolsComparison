...
This pipeline has been built in order to generate the results for our paper : [__DeepG4__: A deep learning approach to predict active G-quadruplexes](https://www.biorxiv.org/content/early/2020/07/23/2020.07.22.215699).
...
DNA sequences into fasta format, splitted into positive set and negative set, writted into the `fasta` directory.
...
* __qG4G4seq__: `Peaks_qG4-ChIP-seq-of-breast-cancer-PDTX_breastCancer_G4seq_hg19_253b_Ctrl_gkmSVM.Fa`  `Peaks_qG4-ChIP-seq-of-breast-cancer-PDTX_breastCancer_G4seq_hg19_253b.Fa`


### Main result
```{r boxplot1, echo=TRUE, fig.height=12, fig.width=16,dev="png",dpi=300,echo=F,warning=F,message=F}
require(ggplot2)
#peaks BG4G4seq
fres <- read_tsv("results/recap_AUC_4.tsv")
cc <- fres %>% filter(TypeExp %in% c("TestSet_Peaks_BG4_G4seq","Peaks_BG4_G4seq","TestSet_Peaks_qG4")) %>% 
  unite(Files,TypeExp,Exp,cell_line,Ctrl) %>%
  group_by(Files) %>% 
  arrange(desc(AUC)) %>% mutate(Classement = 1:dplyr::n()) %>% 
  ungroup() %>% 
  mutate(my_text = str_c(Classement," (",round(AUC,2),")")) %>% 
  mutate(Tool = fct_reorder(Tool,AUC,mean))
cctext <- cc %>% group_by(Tool) %>% summarise(meanscore = mean(AUC),AUC = sum(AUC))

cc %>% 
  ggplot(aes(x=Tool,y=AUC)) +
  geom_bar(aes(fill=Files),stat="identity",col="black") +
  theme_classic(base_size=20) +
  theme(legend.text=element_text(size=8)) +
  coord_flip() +
  # theme(legend.position = "bottom") +
  geom_text(aes(label=my_text), position=position_stack(0.3), size=5)  + ggtitle("AUC on BG4-G4Seq datasets") +
  ylab("sum(AUC)") + xlab("Tools") + ylim(c(0,5)) +
  geom_label(data=cctext,aes(x=Tool,y=AUC,label = round(meanscore,3)),position = position_dodge(width = 1),hjust = -0.1,size=5)-> p4
print(p4)
```
