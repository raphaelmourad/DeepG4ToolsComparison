...
### Main result
```{r boxplot1, echo=TRUE, fig.height=16, fig.width=28,dev="svg",echo=F,warning=F,message=F}
require(ggplot2)
#peaks BG4G4seq
...
cc %>% 
  ...
  geom_text(aes(label=my_text), position=position_stack(0.3), size=8)  + ggtitle("AUC on BG4-G4Seq datasets") +
  ylab("sum(AUC)") + xlab("Tools") +
  geom_label(data=cctext,aes(x=Tool,y=AUC,label = round(meanscore,3)),position = position_dodge(width = 1),hjust = -0.1,size=8)-> p4
print(p4)
```
