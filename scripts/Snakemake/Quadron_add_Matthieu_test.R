...
QuadronSeqList <- function(){
  seqlist <- readDNAStringSet("results/TestSet_Peaks_BG4_G4seq_HaCaT_GSE76688_hg19_201b_Ctrl_gkmSVM_0.8_42/fasta/merged/TestSet_Peaks_BG4_G4seq_HaCaT_GSE76688_hg19_201b_Ctrl_gkmSVM_0.8_42_Ctrl_neg_merged.Fa")
  
  res <- lapply(1:length(seqlist),function(i){message(i);Quadron2(seqlist[i])}) %>% bind_rows(.id = "name")
  
  
}

#Change : remove ReturnOnlyNC = TRUE and NonCanonical = TRUE part code
#Change : remove seq loading because we want a score BY seq and not all potentiel QP given any seq
#Change : remove outfile because we want the output directly
#Change : Add a feature argument to get the feature and retrain when needed
Quadron2 <- function(seq    = NULL,
                     nCPU         = 1,
  ...
  out.data <- data.frame(
    start.pos = QP$start.pos,
    strand = QP$strand,
    seq.length = QP$seq.length,
    score = round(PRED,2)
  )
  return(out.data)
