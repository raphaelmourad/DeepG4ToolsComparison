...
bay_out_1$History %>% write_tsv("results/bay_out_RunKeras_DeepG4_ATAC_rescale_BW_by_bg_5kb_seuil_2_19_04_2021_13_07_21_full.tsv")

p_baye_res <- read_tsv("results/bay_out_RunKeras_DeepG4_ATAC_rescale_BW_by_bg_5kb_seuil_2_19_04_2021_13_07_21_full.tsv")  %>%
  gather(key = "metrics",value = value,-Round,-Value)%>%
  mutate(Best_col = ifelse(Round == 25,"red","black"))%>%
  mutate(Best_size = ifelse(Round == 25,3,1))%>%
  mutate(Best_shape = ifelse(Round == 25,17,16)) %>%
  ...
  facet_wrap(~metrics,nrow=3,scales="free_x") + scale_shape_identity() + xlab("") + theme_classic()
