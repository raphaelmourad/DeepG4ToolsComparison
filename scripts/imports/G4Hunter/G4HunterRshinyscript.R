#G4hunter Code with TABMAX function from Elissar Nassereddine
...
######function Elissar##########"
TabMaxNb <- function(seq=NULL,x=1,y=2,z = 0.1){
  if(is.null(seq))
    return(NULL)
  hl =seq(from = x, to = y, by = z)
  matNb=NULL
  matMaxScore=NULL
  for(hli in hl){
    
    veci <- lapply(seq,function(i){
      res <- modG4huntref(chr = i, hl = hli)
      if(length(res) == 0){
        return(
          data.frame(
            peaks_nb = 0,
        ...
        return(
          data.frame(
            peaks_nb = length(res),
            maxscore = max(score(res))
          )
        )    
      }
    })
    
    mati <- do.call("rbind",veci)
    matNb=cbind(matNb,mati[,1])
    matMaxScore=cbind(matMaxScore,mati[,2])
    listboth <- (list(matNb=matNb,matMaxScore=matMaxScore))
  }
